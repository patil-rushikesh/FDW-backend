...
def fill_template_document(data, user_id):
    # First, get user details
    try:
        user_response = requests.get(f'http://127.0.0.1:5000/users/{user_id}')
        if user_response.status_code == 200:
            # Direct parse from response text - no need for double parsing
    ...
    placeholders = {
        ...
        # ... (rest of your existing placeholders)
    }
    ...
    # Special calculations for startup revenue/funding
    if 'B' in data and 'startupPCCOE' in data['B']:
        if data['B']['startupPCCOE']['revenue']['amount'] > 50000:
            placeholders['{startup_revenue_pccoe_marks}'] = '100'
        if data['B']['startupPCCOE']['funding']['amount'] > 500000:
            placeholders['{startup_funding_pccoe_marks}'] = '100'
...
def generate_filled_document(department, user_id):
    ...
    try:
        ...
        section_a = requests.get(f'http://127.0.0.1:5000/{department}/{user_id}/A').json()
        section_b = requests.get(f'http://127.0.0.1:5000/{department}/{user_id}/B').json()
        section_c = requests.get(f'http://127.0.0.1:5000/{department}/{user_id}/C').json()
        ...
        # Pass user_id to fill_template_document
        doc = fill_template_document(data, user_id)
        ...
        # Send file with proper cleanup handling
        return send_file(
            output_path,
            as_attachment=True,
        ...
        if output_path and os.path.exists(output_path):
            try:
                ...
                pass  # If we can't delete now, it will be handled by cleanup task
        return jsonify({"error": str(e)}), 500
